...
final class RecordsService {
    ...
    ///
    /// - Parameters:
    ///   - recordId: 조회할 활동 기록 ID
    ///   - context: 조회 컨텍스트 (목록 필터 정보)
    /// - Returns: 활동 기록 상세 정보 (prevRecordId, nextRecordId 포함)
    /// - Throws:
    ...
    func fetchRecordDetail(recordId: Int, context: ActivityDetailContext? = nil) async throws -> DTO.ActivityRecordDetailResponse {
        return try await provider.request(.fetchRecordDetail(recordId: recordId, context: context))
    }
